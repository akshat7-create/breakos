...
export function Sidebar({ onNavClick }: { onNavClick?: () => void }) {
    ...
    return (
        <aside className="w-[280px] lg:w-[220px] bg-[var(--sidebar-bg)] border-r border-[var(--border-subtle)] flex flex-col h-full z-20 shadow-xl lg:shadow-none">
            {/* Logo */}
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                "h-11 lg:h-10 px-3 flex items-center gap-3 w-full text-left rounded-lg transition-all duration-150",
                                isActive
                                    ? "bg-[var(--sidebar-active)] text-[var(--text-primary)] font-medium"
                                    : "text-[var(--text-secondary)] hover:bg-[var(--surface-overlay)] hover:text-[var(--text-primary)]"
                            )}
                        >
                            <Icon size={18} className={cn("lg:w-4 lg:h-4", isActive ? "text-[var(--text-primary)]" : "text-[var(--text-muted)]")} />
                            <span className="text-[14px] lg:text-[13px] flex-1">{item.label}</span>
            ...
            <div className="px-4 pb-6 lg:pb-5 pt-4 border-t border-[var(--border-subtle)] shrink-0 space-y-3">
                <div className="bg-[var(--surface)] rounded-xl p-3 shadow-sm lg:shadow-none">
